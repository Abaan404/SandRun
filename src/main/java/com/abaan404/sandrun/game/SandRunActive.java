...
public class SandRunActive {
    private final SandRunConfig config;

    public final GameSpace gameSpace;
    private final SandRunMap gameMap;

    private final Object2ObjectMap<PlayerRef, SandRunPlayer> participants;
    private final SandRunSpawnLogic spawnLogic;
    private final SandRunStageManager stageManager;
    private final boolean ignoreWinState;
    private final SandRunTimerBar timerBar;
    private final ServerWorld world;

    private SandRunActive(GameSpace gameSpace, ServerWorld world, SandRunMap map, GlobalWidgets widgets,
            SandRunConfig config, Set<PlayerRef> participants) {
        ...
        for (PlayerRef player : participants) {
            this.participants.put(player, new SandRunPlayer());
        }

        this.stageManager = new SandRunStageManager();
        this.ignoreWinState = this.participants.size() <= 1;
        this.timerBar = new SandRunTimerBar(widgets);
    }
    ...
    public static void open(GameSpace gameSpace, ServerWorld world, SandRunMap map, SandRunConfig config) {
        gameSpace.setActivity(game -> {
            ...
            SandRunActive active = new SandRunActive(gameSpace, world, map, widgets, config, participants);
    ...
    private void tick() {
        ...
        SandRunStageManager.IdleTickResult result = this.stageManager.tick(time, gameSpace);
